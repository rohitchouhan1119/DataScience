...
class JobsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for listing in listings:
        	...
        	'''
        	Meta will transfer our data from parse method to parse_listing method == yield


        	Below code can be used in place of above yield request when we dont want to go to individual link to extarct data
        	OR
        	yield{'Date': date,
        		  'Link': link,
        		  'Text': text	}  
         	'''
        # multiple pages
        # object of next page will be available for start url so callback will be parse only
